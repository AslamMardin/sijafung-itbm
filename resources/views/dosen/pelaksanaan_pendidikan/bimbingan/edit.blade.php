@extends('layouts.app')
@section('title', 'Edit Bimbingan Mahasiswa')
@section('page-title', 'Edit Bimbingan Mahasiswa')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Form Edit Bimbingan Mahasiswa</h3>
        <a href="{{ route('dosen.pendidikan.index') }}" class="btn btn-outline btn-sm">
            <i class="fas fa-arrow-left"></i> Kembali
        </a>
    </div>

    <form action="{{ route('dosen.pendidikan.update', ['bimbingan', $pendidikan]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="row">
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" name="judul_bimbingan" id="judul_bimbingan" class="form-control @error('judul_bimbingan') is-invalid @enderror"
                       value="{{ old('judul_bimbingan', $pendidikan->judul_bimbingan) }}" required placeholder="Judul skripsi/tesis/disertasi">
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <select name="jenis_bimbingan" id="jenis_bimbingan" class="form-control">
                            ...
                            <option value="Skripsi" {{ old('jenis_bimbingan', $pendidikan->jenis_bimbingan) == 'Skripsi' ? 'selected' : '' }}>Skripsi</option>
                            <option value="Tesis" {{ old('jenis_bimbingan', $pendidikan->jenis_bimbingan) == 'Tesis' ? 'selected' : '' }}>Tesis</option>
                            <option value="Disertasi" {{ old('jenis_bimbingan', $pendidikan->jenis_bimbingan) == 'Disertasi' ? 'selected' : '' }}>Disertasi</option>
