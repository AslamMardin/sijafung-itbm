@extends('layouts.app')
@section('title', 'Input Publikasi Karya')
@section('page-title', 'Input Publikasi Karya')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Form Input Publikasi Karya</h3>
        <a href="{{ route('dosen.penelitian.index') }}" class="btn btn-outline btn-sm">
            <i class="fas fa-arrow-left"></i> Kembali
        </a>
    </div>

    <form action="{{ route('dosen.penelitian.store', 'publikasi_karya') }}" method="POST">
        @csrf
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" name="judul_kegiatan" id="judul_kegiatan" class="form-control @error('judul_kegiatan') is-invalid @enderror"
                       value="{{ old('judul_kegiatan') }}" required placeholder="Judul publikasi karya">
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <select name="jenis_publikasi" id="jenis_publikasi" class="form-control @error('jenis_publikasi') is-invalid @enderror" required>
                            ...
                            <option value="Buku referensi" {{ old('jenis_publikasi') == 'Buku referensi' ? 'selected' : '' }}>Buku Referensi</option>
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <select name="peran_penulis" id="peran_penulis" class="form-control @error('peran_penulis') is-invalid @enderror" required>
                            ...
                            <option value="penulis" {{ old('peran_penulis') == 'penulis' ? 'selected' : '' }}>Penulis</option>
                            <option value="editor" {{ old('peran_penulis') == 'editor' ? 'selected' : '' }}>Editor</option>
                            <option value="penerjemah" {{ old('peran_penulis') == 'penerjemah' ? 'selected' : '' }}>Penerjemah</option>
                        </select>
                        @error('peran_penulis')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
