@extends('layouts.app')
@section('title', 'Pelaksanaan Pengabdian')
@section('page-title', 'Pelaksanaan Pengabdian')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Daftar Kegiatan Pengabdian</h3>
        @if($jenisKegiatan)
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <small class="text-muted">{{ $kegiatan->afiliasi ?? $kegiatan->penyelenggara ?? '-' }}</small>
                        </td>
                        <td>{{ ucwords($kegiatan->peran ?? $kegiatan->peran_jurnal ?? '-') }}</td>
                        ...
                        <td>
                            <div class="btn-group">
                                @if($kegiatan->status == 'Pending')
                                    <a href="{{ route('dosen.pengabdian.edit', [$kegiatan->jenis_kegiatan, $kegiatan]) }}" class="btn btn-sm btn-outline">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                @endif
                                <form action="{{ route('dosen.pengabdian.destroy', $kegiatan) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-4">
                            <div class="empty-state">
                                ...
                                <h3>Belum ada kegiatan pengabdian</h3>
                                <p>Mulai input kegiatan pengabdian Anda melalui menu di sidebar</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
