@extends('layouts.app')
@section('title', 'Edit Pengelola Jurnal')
@section('page-title', 'Edit Pengelola Jurnal')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Form Edit Pengelola Jurnal</h3>
        <a href="{{ route('dosen.pengabdian.index') }}" class="btn btn-outline btn-sm">
            <i class="fas fa-arrow-left"></i> Kembali
        </a>
    </div>
    
    <form action="{{ route('dosen.pengabdian.update', ['pengelola_jurnal', $pengabdian]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <input type="text" name="nama_jurnal" id="nama_jurnal" class="form-control @error('nama_jurnal') is-invalid @enderror" 
                               value="{{ old('nama_jurnal', $pengabdian->nama_jurnal) }}" required>
                        @error('nama_jurnal')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <input type="text" name="no_sk_penugasan" id="no_sk_penugasan" class="form-control" 
                               value="{{ old('no_sk_penugasan', $pengabdian->no_sk_penugasan) }}">
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <select name="peran_jurnal" id="peran_jurnal" class="form-control @error('peran_jurnal') is-invalid @enderror" required>
                            ...
                            <option value="Editor" {{ old('peran_jurnal', $pengabdian->peran_jurnal) == 'Editor' ? 'selected' : '' }}>Editor</option>
                            <option value="Dewan Penyunting" {{ old('peran_jurnal', $pengabdian->peran_jurnal) == 'Dewan Penyunting' ? 'selected' : '' }}>Dewan Penyunting</option>
                            <option value="Dewan Redaksi" {{ old('peran_jurnal', $pengabdian->peran_jurnal) == 'Dewan Redaksi' ? 'selected' : '' }}>Dewan Redaksi</option>
                        </select>
                        @error('peran_jurnal')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <input type="date" name="terhitung_mulai" id="terhitung_mulai" class="form-control @error('terhitung_mulai') is-invalid @enderror" 
                               value="{{ old('terhitung_mulai', $pengabdian->terhitung_mulai?->format('Y-m-d')) }}" required>
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <input type="date" name="tanggal_selesai" id="tanggal_selesai" class="form-control" 
                               value="{{ old('tanggal_selesai', $pengabdian->tanggal_selesai?->format('Y-m-d')) }}">
            ...
            <div class="form-group mb-3">
                <div class="form-check">
                    ...
                    <input type="checkbox" name="status_aktif" id="status_aktif" class="form-check-input" value="1" {{ old('status_aktif', $pengabdian->status_aktif) ? 'checked' : '' }}>
